...
var MFP = {
	...
	_initModule: function( $module, data ) {
		...
		$module.find('[data-name="inline-horizontal"]')
			.append( self.createField( 'radio_group', '[inline_horizontal]', typeof data['inline_horizontal'] != 'undefined' && data['inline_horizontal'] ? true : false, {
				'multiOptions' : {
					'items' : [
						[ '0', '<?php echo $text_no; ?>' ],
						[ '1', '<?php echo $text_yes; ?>' ]
		...
		$module.find('[data-name="status"]')
			.append( self.createField( 'radio_group', '[status]', typeof data['status'] == 'undefined' ? true : data['status'], {
				'multiOptions' : {
					'items' : [
